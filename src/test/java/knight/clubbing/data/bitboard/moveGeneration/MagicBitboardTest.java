...
import static knight.clubbing.data.bitboard.moveGeneration.magic.Magic.getBishopIndex;
...
public class MagicBitboardTest {
    ...
    void testRookAndBishopAttackCorrectness() {
        ...
        for (int square = 0; square < 64; square++) {
            for (int i = 0; i < 10; i++) {
                long blockers = rng.nextLong() & Magic.RookMask[square];  // only relevant blockers
    ...
    void testFullyBlocked() {
        for (int square = 0; square < 64; square++) {
            long rookAttacks = Magic.getRookAttacks(square, ~0L);
            long bishopAttacks = Magic.getBishopAttacks(square, ~0L);
            assertTrue(rookAttacks != 0);  // Should still reach 1 square out
    ...
    void testSymmetry() {
        for (int square = 0; square < 32; square++) {
            ...
            long blockers = 0x0000001818000000L; // blockers around center
    ...
    void testNoMagicCollision() {
        for (int square = 0; square < 64; square++) {
            ...
            for (long b : blockers) {
                ...
                long index = Magic.getRookIndex(square, b); // helper: compute used index
